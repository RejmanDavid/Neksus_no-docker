...
public class FilesDAO {
    ...
    public List<Files> getFilesByModId(Long modId) {
        String sql = "SELECT * FROM FILES WHERE MOD_ID = ?";
        return jdbcTemplate.query(sql, filesRowMapper, modId);
    }

    public boolean insertFile(Files file) {
        String sql = "INSERT INTO FILES (FILE_ID, MOD_ID, DESCRIPTION, FILE_PATH, RELEASE_DATE, VERSION) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public boolean updateFile(Files file) {
        String sql = "UPDATE FILES SET MOD_ID = ?, DESCRIPTION = ?, FILE_PATH = ?, RELEASE_DATE = ?, VERSION = ? WHERE FILE_ID = ?";
